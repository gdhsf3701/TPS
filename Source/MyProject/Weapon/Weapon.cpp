...
void AWeapon::StartFire(TWeakObjectPtr<class ATPSCharacter> OwnerCharacter)
{
	...
	if (Character) {
		...
		case EFireType::EF_Projectile:
			GetWorldTimerManager().SetTimer(FireTimerHandle, [Character, this]() {FireWithProjectile(Character); }, FireInterval, true);
			break;
		case EFireType::EF_LineTrace:
			GetWorldTimerManager().SetTimer(FireTimerHandle, [Character, this]() {FireWithLineTrace (Character); }, FireInterval, true);
			break;
		default:
			break;
		}
...
void AWeapon::FireWithProjectile(TWeakObjectPtr<class ATPSCharacter> OwnerCharacter)
{
	...
	FTransform FireTransfrom = MeshComponent->GetSocketTransform("FireSocket");
	FVector Direction = FireTransfrom.GetRotation().GetForwardVector() * TraceDistance;
